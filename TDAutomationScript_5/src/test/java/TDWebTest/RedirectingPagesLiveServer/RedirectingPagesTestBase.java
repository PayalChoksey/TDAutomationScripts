package TDWebTest.RedirectingPagesLiveServer;
...
public class RedirectingPagesTestBase extends TDWeb_007_Combine{
	...
	public void differentbrowser(String browser) throws IOException {
		...
		if (browser.equals("Chrome"))
		{
			...
			test = report.startTest("TD Registration Page - In Chrome", "Opening the Application on Live Server");
			...
			tdweb.get(Config.getProperty("LiveURL"));
			UtilityRedirectingPage.capturescreenshot(tdweb, "ChromeApplication on Live Server");
			test.log(LogStatus.INFO, "Time Doctor Application Opened in Chrome on Live Server");
			test.log(LogStatus.PASS, "Application opened sucessfully in Chrome on Live Server");
			report.endTest(test);
			report.flush();
		}
		...
		{	
			...
			test = report.startTest("TD Registration Page - In FireFox", "Opening the Application on Live Server");
			...
			tdweb.manage().window().fullscreen();
			tdweb.get(Config.getProperty("LiveURL"));
			UtilityRedirectingPage.capturescreenshot(tdweb, "FirefoxApplication on Live Server");
			test.log(LogStatus.INFO, "Time Doctor Application Opened in Firefox on Live Server");
			test.log(LogStatus.PASS, "Application opened sucessfully in Firefox on Live Server");
			report.endTest(test);
			report.flush();
		}
		else if (browser.equals("Safari"))
		{
			...
			test = report.startTest("TD Registration Page - In Safari", "Opening the Application on Live Server");
			tdweb = new SafariDriver();
			tdweb.manage().window().maximize();
			tdweb.get(Config.getProperty("LiveURL"));
			UtilityRedirectingPage.capturescreenshot(tdweb, "SafariApplication on Live Server");
			test.log(LogStatus.INFO, "Time Doctor Application Opened in Safari on Live Server");
			test.log(LogStatus.PASS, "Application opened sucessfully in Safari on Live Server");
			report.endTest(test);
			report.flush();
		}
