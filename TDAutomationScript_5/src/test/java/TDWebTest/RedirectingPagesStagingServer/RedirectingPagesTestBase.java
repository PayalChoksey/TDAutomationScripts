package TDWebTest.RedirectingPagesStagingServer;
...
public class RedirectingPagesTestBase extends TDWeb_007_Combine{
	...
	public void differentbrowser(String browser) throws IOException {
		...
		{
			report = new ExtentReports(System.getProperty("user.dir")+"/src//test//resources//Reports//RedirectingPagesStagingServer//RedirectingPages1.html", true);
			test = report.startTest("TD Registration Page - In Chrome", "Opening the Application on Staging Server");
			...
			tdweb.get(Config.getProperty("StagingURL"));
			UtilityRedirectingPage.capturescreenshot(tdweb, "ChromeApplication on Staging Server");
			test.log(LogStatus.INFO, "Time Doctor Application Opened in Chrome on Staging Server");
			test.log(LogStatus.PASS, "Application opened sucessfully in Chrome on Stagig Server");
			report.endTest(test);
			report.flush();
		}
		else if(browser.equals("Firefox"))
		{	
			report = new ExtentReports(System.getProperty("user.dir")+"/src//test//resources//Reports//RedirectingPagesStagingServer//RedirectingPages2.html", true);
			test = report.startTest("TD Registration Page - In FireFox", "Opening the Application on Staging Server");
			...
			tdweb.manage().window().fullscreen();
			tdweb.get(Config.getProperty("StagingURL"));
			UtilityRedirectingPage.capturescreenshot(tdweb, "FirefoxApplication on Staging Server");
			test.log(LogStatus.INFO, "Time Doctor Application Opened in Firefox on Staging Server");
			test.log(LogStatus.PASS, "Application opened sucessfully in Firefox on Staging Server");
			report.endTest(test);
			report.flush();
		}
		else if (browser.equals("Safari"))
		{
			report = new ExtentReports(System.getProperty("user.dir")+"/src//test//resources//Reports//RedirectingPagesStagingServer//RedirectingPages3.html", true);
			test = report.startTest("TD Registration Page - In Safari", "Opening the Application on Staging Server");
			tdweb = new SafariDriver();
		//	tdweb.manage().window().fullscreen();
			tdweb.get(Config.getProperty("LoginURL"));
			UtilityRedirectingPage.capturescreenshot(tdweb, "SafariApplication on Staging Server");
			test.log(LogStatus.INFO, "Time Doctor Application Opened in Safari on Staging Server");
			test.log(LogStatus.PASS, "Application opened sucessfully in Safari on Staging Server");
			report.endTest(test);
			report.flush();
		}
